...
TradfriRangeExtender::TradfriRangeExtender(ZigbeeNetwork *network, ZigbeeAddress ieeeAddress, Device *device, QObject *parent) :
...
void TradfriRangeExtender::checkOnlineStatus()
{
    if (m_network->state() == ZigbeeNetwork::StateRunning) {
        device()->setStateValue(tradfriRangeExtenderConnectedStateTypeId, true);
        device()->setStateValue(tradfriRangeExtenderVersionStateTypeId, m_endpoint->softwareBuildId());
    } else {
        device()->setStateValue(tradfriRangeExtenderConnectedStateTypeId, false);
    }
}
